...
class FaceI2C():
    ...
    def __init__(self, dir = "/", SDA_PIN = 25, SCL_PIN = 26, FREQ_I2C = 5000, gav = False, scale = 0, freqAmostra = 250):
        ...
        #print("\n=> Escala de aceleração : ", self.setScale)
        ...
        #print("\n=> Frequencia de amostragem : ", (1000/freqAmostra), "Hz")
        #print("\n=> Tempo estipulado para dormir: ", (1024/((1000/freqAmostra)*6)), " s\n")
    ...
    def pega_valor(self):
        ...
        for i in range(170):
            ...
            AccX[i] = (self.twos_comp((valores[0 + (6*i)] << 8 | valores[1 + (6*i)])) * self.convToGav) - self.accel_bias[0]
            AccY[i] = (self.twos_comp((valores[2 + (6*i)] << 8 | valores[3 + (6*i)])) * self.convToGav) - self.accel_bias[1] 
            AccZ[i] = (self.twos_comp((valores[4 + (6*i)] << 8 | valores[5 + (6*i)])) * self.convToGav) - self.accel_bias[2]
    ...
    def calibrateMPU6050(self):
        ...
        self.accel_bias[0] = self.accel_bias[0] / self.LSB
        self.accel_bias[1] = self.accel_bias[1] / self.LSB
        self.accel_bias[2] = self.accel_bias[2] / self.LSB
